...
TIM_timer_start(
  ...
  const Escher_uSec_t ee_milliseconds )
{
  /* Insert implementation specific code here.  */
  ETimer_t * t = timer_start( ee_milliseconds, ee_event_inst );
  ...
  const Escher_Timer_t ee_timer_inst )
{
  ...
  if ( ( 0 != t ) && ( ee_timer_inst.key == t->accesskey ) ) {
    rv = ETimer_msec_time();
    rv = ( t->expiration > rv ) ? ( t->expiration - rv ) : 0UL;
  }
  return ( rv );  
}
  ...
  const Escher_Timer_t ee_timer_inst )
{
  ...
  if ( ( 0 != t ) && ( ee_timer_inst.key == t->accesskey ) && ( t->expiration > 0UL ) ) {
    t->expiration = ETimer_msec_time() + ee_milliseconds + 1UL;
